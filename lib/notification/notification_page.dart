...
class _LoanCalendarPageState extends State<LoanCalendarPage> {
  ...
  final List<Loan> _loans = [
    Loan(
      ...
      endDate: DateTime.now().add(const Duration(hours: 10)), // 10 jam lagi
    ),
    Loan(
      id: '2',
      itemName: 'Kamera Canon EOS',
      startDate: DateTime.now().subtract(const Duration(days: 5)),
      endDate: DateTime.now().add(const Duration(days: 2, hours: 6)),
    ),
    Loan(
      id: '3',
      itemName: 'Buku Flutter Advanced',
      startDate: DateTime.now().subtract(const Duration(days: 1)),
      endDate: DateTime.now().add(const Duration(hours: 18)), // 18 jam lagi
    ),
  ];
  ...
  // Mendapatkan notifikasi yang akan berakhir dalam 12 jam
  List<Loan> get _upcomingLoans {
    final now = DateTime.now();
    return _loans.where((loan) {
      final timeLeft = loan.endDate.difference(now);
      return timeLeft.inHours <= 12 && timeLeft.inSeconds > 0;
    }).toList();
  }
      ...
      body: Column(
        children: [
          ...
          TableCalendar(
            ...
            eventLoader: (day) {
              return _loans
                  .where((loan) => isSameDay(loan.endDate, day))
                  .toList();
            },
            ...
            child: _upcomingLoans.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      final loan = _upcomingLoans[index];
